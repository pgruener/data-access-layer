...
var DataModel = /** @class */ (function () {
    function DataModel(properties, dataProvider, isNewInstance) {
        ...
        this.creationScheduled = false; // Bei neu angelegter entity, auf der save() aufgerufen wurde
        this.markedForDeletion = false;
        this.transactionRunning = false;
        this.dataModelRequests = [];
        this._pendingObjectMaps = [];
        this.getClientId = function () { return _this.getProperty(internal_1.CLIENT_ID_ATTRIBUTE); };
        this.shouldMerge = function (objectMap) {
            ...
            return new Date(_this.getProperty(updatedAtAttributeName)).getTime() < new Date(objectMap[updatedAtAttributeName].toString()).getTime();
        };
        this.removeFinishedDataModelRequests = function () {
            _this.dataModelRequests = _this.dataModelRequests.filter(function (dataModelRequestData) { return !dataModelRequestData.isFinished(); });
        };
        this.unprocessedDataModelRequests = function () {
            ...
            // }
            // return results
            return _this.dataModelRequests.filter(function (dataModelRequest) { return !dataModelRequest.isFinished(); });
        };
        ...
        this.setCreationScheduled = function () {
            if (_this.creationScheduled) {
                return false;
            }
            return _this.creationScheduled = true;
        };
        this.informAboutDataModelRequestData = function (dataModelRequestData) {
            _this.dataModelRequests.push(dataModelRequestData);
        };
    ...
    DataModel.prototype.getProperty = function (propertyName, fallbackValue) {
        ...
        var property = this.properties.getValue(propertyName);
    ...
    DataModel.prototype.mergeChanges = function (objectMap) {
        ...
        var anythingChanged = false;
        var pendingObjectMap = null;
        while (pendingObjectMap = this._pendingObjectMaps.shift()) {
            var shouldMerge = this.shouldMerge(objectMap);
            if (shouldMerge) {
                Object.keys(pendingObjectMap).forEach(function (propertyName) {
                    if (!_this.properties.hasProperty(propertyName, pendingObjectMap[propertyName])) {
                        _this.clientChangedProperties.removeProperty(propertyName);
                        _this.properties.set(propertyName, pendingObjectMap[propertyName]);
     ...
     */
    DataModel.prototype.computeIdentityHashCode = function (computeOnlyWithClientIdAttribute) {
     ...
     */
    DataModel.computeIdentityHashCode = function (dataModel, dataProviderConfig, computeOnlyWithClientIdAttribute) {
        var identityHashCode = dataProviderConfig.dataProviderName;
        // special handling for !Client-Only! unpersisted entities
        if (dataModel instanceof DataModel && !dataModel.isPersisted()) {
            return identityHashCode += '_' + dataModel.getProperty(internal_1.CLIENT_ID_ATTRIBUTE);
        }
        // TODO: put this in extra method, which can be overloaded in inheriting model
        var attributeNames = computeOnlyWithClientIdAttribute ? [] : dataProviderConfig.getIdentityRelevantAttributeNames();
